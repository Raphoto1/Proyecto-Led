...
function crecerPantalla() {
  let crecer = parseInt(
    prompt("Cuantas veces quieres hacer crecer tu pantalla?")
  );
  ...
  while (crecer == 0) {
    crecer = parseInt(
      prompt("Cuantos Pixeles tienen tus modulos en X de 50cmx50cm?")
    );
    if (Number.isNaN(crecer)) {
      let convertirNot = borrarNan(crecer);
      crecer = convertirNot;
  ...
  for (let i = 1; i <= crecer; i++) {
    ...
    alert(
      "tu pantalla ahora seria de " +
        modulosX +
        " modulos en X " +
        modulosY +
        " modulos en Y, necesitarias un total de " +
        modulos +
        " modulos, el total de pixeles en x, " +
        tamanoPixelesY +
        " cuando sea " +
        i +
        " veces mas grande"
...
//pedir data parte 1
medidaPixelesModulo = parseInt(
  prompt("Cuantos Pixeles tienen tus modulos en X de 50cmx50cm?")
);
tamanoTotalCmX = parseInt(
  prompt("de que tamaño sera tu pantalla en cm en largo (Minimo 50cm)")
);
tamanoTotalCmY = parseInt(
  prompt("de que tamaño sera tu pantalla en cm en Alto (Minimo 50cm)")
);
...
while (tamanoTotalCmX == 0) {
  tamanoTotalCmX = parseInt(
    prompt("de que tamaño sera tu pantalla en cm en largo")
  );
  if (Number.isNaN(tamanoTotalCmX)) {
    let convertirNot = borrarNan(tamanoTotalCmX);
    tamanoTotalCmX = convertirNot;
...
while (tamanoTotalCmY == 0) {
  tamanoTotalCmY = parseInt(
    prompt("de que tamaño sera tu pantalla en cm en Alto")
  );
  if (Number.isNaN(tamanoTotalCmY)) {
    let convertirNot = borrarNan(tamanoTotalCmY);
    tamanoTotalCmY = convertirNot;
...
if (medidaPixelesModulo > 0) {
  ...
  alert(
    "el total de pixeles que debe sopoortar tu equipo es " + tamanoPixelesTotal
  );
} else {
  alert("no me has dado pixeles en X de tu modulo 50x50");
}
//preguntar si quiere cambiar
let crecerSiONo = parseInt(
  prompt("Quieres hacer crecer tu pantalla?, 1.Si, 2.No")
);
